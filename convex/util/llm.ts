...
// The Convex vector index has a fixed dimension. The configured online API can
// omit an embedding endpoint by using the deterministic `local-hash` fallback.
...
export function getLLMConfig(): LLMConfig {
  ...
  if (!apiUrl) {
    throw new Error(
      "LLM_API_URL is required. Configure the online API with: npx convex env set LLM_API_URL 'https://your-provider.example/v1'",
    );
  }
  const chatModel = process.env.LLM_MODEL?.trim();
  if (!chatModel) {
    throw new Error(
      "LLM_MODEL is required. Configure it with: npx convex env set LLM_MODEL 'your-model'",
    );
  }
  return {
    url: normalizeApiBaseUrl(apiUrl),
    chatModel,
    embeddingModel: process.env.LLM_EMBEDDING_MODEL?.trim() || 'local-hash',
    stopWords: [],
