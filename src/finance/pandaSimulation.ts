import { assessTradeRisk, TOWN_TRADERS } from '../../shared/finance';
import type {
  CausalEvent,
  DashboardError,
  DashboardExecution,
...
export function buildPandaSimulationRun(market: DashboardMarket): PandaSimulationRun {
  ...
  const traders = TOWN_TRADERS.filter((profile) => profile.kind === 'ai').map((profile, index) => {
    ...
    const evidence = [
      `PANDA:get_stock_daily:${market.symbol}:${features.date}`,
  ...
  return [
    {
      ...
      proof: `PANDA:get_stock_daily:${market.symbol}:${features.date}`,
    },
    {
      id: `${runId}-indicators`,
      time: '15:00:02',
